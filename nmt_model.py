...
class NMT(nn.Module):
    """ Simple Neural Machine Translation Model:
        ...
        additive_attention
    """
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2,attention_type = 'additive_attention', self_attention=False):
        """ Init NMT Model.
        ...
        self.Wproj_1 = nn.Linear(in_features = hidden_size, out_features = hidden_size, bias = False)
        self.Wproj_2 = nn.Linear(in_features = hidden_size, out_features = hidden_size, bias = False)
        self.scale_V = nn.Linear(in_features = hidden_size, out_features = 1, bias = False)
        self.tanh = nn.Tanh()
        self.W_Mul = nn.Linear(in_features = hidden_size, out_features = hidden_size, bias = False)
        self.v_self = nn.Linear(in_features = hidden_size, out_features = 1, bias = False)
        self.W_self = nn.Linear(in_features = hidden_size, out_features = hidden_size, bias = False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #print("enc_hidden: ",enc_hiddens.size(),"\n")
        ...
        if self.self_attention :
            e_t = self.v_self(self.tanh(self.W_self(enc_hiddens)))
        ...
        init_decoder_cell = self.h_projection(torch.cat((last_cell[0],last_cell[1]),1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #print("enc_hidden_proj: ",enc_hiddens_proj.size(),"\n")
        Y = self.model_embeddings.target(target_padded)
        #o_prev = torch.zeros(enc_hiddens.size(2)//2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        if self.attention_type == 'additive_attention':
            e_t = self.scale_V(self.tanh(self.Wproj_1(dec_hidden).unsqueeze(1) + self.Wproj_2(enc_hiddens_proj))).squeeze(2)
        elif self.attention_type == 'Dot_Scaled_attention':
            e_t = torch.squeeze((torch.bmm(torch.unsqueeze(dec_hidden, dim =1 ) , enc_hiddens_proj.transpose(1,2))),1)
        elif self.attention_type == 'multiplicative_attention':
            hidden_layer_M = torch.tanh((self.W_Mul(enc_hiddens_proj)))
            e_t = torch.squeeze((torch.bmm(torch.unsqueeze(dec_hidden, dim =1 ) , hidden_layer_M.transpose(1,2))),1)
        elif self.attention_type == 'key_value':
        ...
        ###
        ###       Hints:
        ###         - dec_hidden is shape (b,1, h) and corresponds to h^dec_t in the PDF (batched)
